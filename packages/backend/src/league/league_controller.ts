...
export class LeaguesController extends ControllerWrap<League> {
      ...
      this.withUserId(req, async (currUser) => {
        ...
        if (creation.private) {
          league = {
            ...
            private: true,
            ownerIds: [currUser],
            round: 0, //TODO: creation needs to be updated to initialise the league at round 0
  ...
  public async generateMatchProposals(
    ...
    return this.withUser(req, async (currUser) => {
      ...
      const users = await (await getDb().users())
        .find({ leagues: league._id })
        .toArray();

      // TODO: Should be autogenerated from the AbilityLevel enum
      // const peopleByAbilityLevel = Object.fromEntries(
      //   Object.keys(AbilityLevel).map((k) => [k, [] as ObjectId[]]),
      // ) as Record<AbilityLevel, ObjectId[]>;
      ...
      for (const user of users) {
        for (const sportInfo of user.sports) {
          if (!peopleBySportAndAbilityLevel[sportInfo.sport]) {
            peopleBySportAndAbilityLevel[sportInfo.sport] = {
              beginner: [],
              intermediate: [],
              advanced: [],
            };
      ...
      for (const sport in Sport) {
        for (const abilityLevel in AbilityLevel) {
          const people = peopleBySportAndAbilityLevel[sport][abilityLevel];
