...
export class LeaguesController extends ControllerWrap<League> {
  ...
  public async generateMatchProposals(
    ...
    return this.withUser(req, async (currUser) => {
      ...
      const users = await (await getDb().users()).find({ leagues: league._id });
      ...
      for (const user of users) {
        peopleByAbilityLevel[user.abilityLevel].push(user._id);
      }

      // Generate match proposals for each ability level
      //TODO: abilityLevel does not appear on the person as it is dependent on the sport. (See User.sports)
      for (const abilityLevel in AbilityLevel) {
        const people =
          peopleByAbilityLevel[abilityLevel as keyof typeof AbilityLevel];
